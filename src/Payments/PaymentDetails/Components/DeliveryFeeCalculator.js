...
export const getFinalDeliveryFee = async (userLat, userLng, cartAmount) => {
  console.log("userLat",userLat,"userLng",userLng,"cart Amount",cartAmount);
  ...
  const deliveryFee = getFeeFromSlab(nearestStore.distance, deliverySlabs, [
    "min_km",
    "max_km",
  ]);
  const handlingFee = getFeeFromSlab(
    cartAmount,
    handlingSlabs,
    ["min_cart", "max_cart"],
