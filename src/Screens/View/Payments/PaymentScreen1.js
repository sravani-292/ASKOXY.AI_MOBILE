...
import DeliveryTimelineModal from "./PaymentDetails/Components/DeliveryModal";
import GoogleAnalyticsService from "../../../Components/GoogleAnalytic";
import RadioGroup from "react-native-radio-buttons-group";
import TimeSlotModal from "./PaymentDetails/Components/TimeSlotModal ";
...
import PaymentType from "./PaymentDetails/Components/PaymentType";
import BillDetails from "./PaymentDetails/Components/PaymentDetails";
...
const PaymentDetails = ({ navigation, route }) => {
  ...
  const totalCart = async () => {
    ...
    try {
      ...
      const totalGoldGst = cartResponse.reduce(
        (sum, item) => sum + (item?.goldGst || 0),
        0
      );
      setGoldGstAmont(totalGoldGst);
      const totalGoldMakingCost = cartResponse.reduce(
                ...
                <View key={index} style={styles.couponItem}>
                  ...
                  <View style={styles.couponActions}>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        copyToClipboard(coupon.couponCode, coupon.maxDiscount)
                      }
                    >
