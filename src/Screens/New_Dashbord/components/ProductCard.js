...
const ProductCard = ({ item, onAddToCart, cartItem, onDecrement, discount, customerId, dynamicContent }) => {
  ...
  const calculations = useMemo(() => {
    const savings = item.itemMrp > item.itemPrice ? item.itemMrp - item.itemPrice : 0;
    const formattedWeight = `${item.weight}${formatUnit(item.weight, item.units)}`;
  ...
  const handleAction = useCallback(async (action, type) => {
    if (loading) return;
  ...
  const handleDecrement = useCallback(() => {
    if (!cartItem) return;
    handleAction(() => onDecrement(cartItem.cartId, customerId, cartItem.cartQuantity, cartItem.itemId), 'decrement');
      ...
      >
        ...
        >
          <Image 
            ...
            style={styles.image} 
            resizeMode="contain"
            defaultSource={require('../../../../assets/SVG/askoxy.png')} // Add placeholder
          />
