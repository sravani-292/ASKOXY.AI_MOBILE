...
const CategorySection = ({
  ...
  useEffect(() => {
    ...
    //   setIsLoading(loadingPriorityItems);
    // }
    if(categories){
      setIsLoading(false);
    }
  ...
  const sortItemsWithPriority = useCallback((items, priorityItems, priorityItemsByName) => {
    ...
    const priorityItemsFiltered = items.filter(item => prioritySet.has(item.itemId));
    const regularItems = items.filter(item => !prioritySet.has(item.itemId));
    ...
    const sortedPriorityItems = priorityItemsFiltered.sort((a, b) => {
      ...
      const aNameIndex = priorityItemsByName.findIndex(name => 
        a.itemName?.toLowerCase().includes(name.toLowerCase()) ||
        a.itemTitle?.toLowerCase().includes(name.toLowerCase())
      );
      const bNameIndex = priorityItemsByName.findIndex(name => 
  ...
  const filteredCategories = useMemo(() => {
    return processedCategories.map(category => {
      ...
      items = sortItemsWithPriority(items, priorityItems, priorityItemsByName);
  ...
  }, [processedCategories, selectedWeightFilter, priorityItems, priorityItemsByName, filterItemsByWeight, sortItemsWithPriority]);
  ...
  const handleSeeAll = useCallback(async (category) => {
    if (onSeeAll) {
  ...
  const handleAddToCart = useCallback(async (item, type) => {
    if (customerId) {
      console.log(item, "...........", type);
      try {
        await addToCart(item, type);
      } catch (error) {
        console.error('Error adding to cart selected: ', error);
      }
    } else {
      Alert.alert(
        "Login Required",
  ...
  const handleRemoveFromCart = useCallback(async (cartId, customerId, quantity, itemId) => {
    try {
      if (quantity === 1) {
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>
          {loadingPriorityItems ? 'Loading priority items...' : 'Loading categories...'}
        </Text>
      </View>
    );
  }
  ...
  return (
    <View key={categories?.categoryType || 'categories'}>
          ...
          >
            {weightRanges.map((range, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterChip,
                  selectedWeightFilter === range.value && styles.filterChipSelected
                ]}
                onPress={() => handleWeightFilterChange(range.value)}
              >
                <Text style={[
                  styles.filterChipText,
                  selectedWeightFilter === range.value && styles.filterChipTextSelected
                ]}>
                  {range.label}
                </Text>
              </TouchableOpacity>
      ...
      {loadingPriorityItems && processedCategories.length > 0 && (
        <View style={styles.overlayLoadingContainer}>
          <View style={styles.overlayLoader}>
            ...
            <Text style={styles.overlayLoadingText}>Updating priority items...</Text>
          </View>
        </View>
      )}
      ...
      {filteredCategories.map((category) => {
        ...
        const itemsToShow = isExpanded ? category.itemsResponseDtoList : 
          ...
          <View key={category.categoryName} style={styles.section}>
            <View style={styles.header}>
                ...
                <View style={styles.titleContainer}>
                  ...
                  <View style={styles.itemCountContainer}>
                    <Text style={styles.itemCount}>
                      {isExpanded ? `${totalItems} items` : `${itemsToShow.length}${shouldShowSeeAll ? `/${totalItems}` : ''} items`}
                    </Text>
                    {selectedWeightFilter && category.originalItemCount !== totalItems && (
              ...
              {shouldShowSeeAll && (
                <TouchableOpacity
                  ...
                  style={[styles.seeAllButton, isLoadingCategory && styles.disabledButton]}
                >
                  <Text style={[styles.seeAll, isLoadingCategory && styles.disabledText]}>
            ...
            >
              {itemsToShow.map((item, index) => {
                ...
                const isPriorityByName = priorityItemsByName.some(name => 
                  item.itemName?.toLowerCase().includes(name.toLowerCase()) ||
                  item.itemTitle?.toLowerCase().includes(name.toLowerCase())
                );
